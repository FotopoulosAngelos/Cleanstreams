# main.py - CleanStreams (COMPLETE + PLAYABLE .MP4 + DRAG MARKERS)
import sys
import sqlite3
import os
import math
...
from PyQt5.QtGui import QPixmap, QFont, QImage, QIcon, QDrag  # QDrag HERE
...
# ----------------------------------------------------------------------
# Draggable Marker (drag-and-drop from sidebar)
# ----------------------------------------------------------------------
...
class Homepage(QWidget):
    def __init__(self, parent=None, username=""):
        ...
        m = folium.Map(location=[38.290427, 21.794836], zoom_start=12)
        data = BytesIO()
...
class MapBridge(QObject):
    ...
    def mapDoubleClicked(self, lat, lng):
        self.double_clicked.emit(lat, lng)

    @pyqtSlot(str, float, float)
    def markerMoved(self, kind, lat, lng):
        self.marker_moved.emit(kind, lat, lng)

    @pyqtSlot(str, float, float)
    def markerDropped(self, kind, lat, lng):
        self.drop_received.emit(kind, lat, lng)

# ----------------------------------------------------------------------
# Route Page – DRAG MARKERS + LIVE UPDATE
# ----------------------------------------------------------------------
class RoutePage(QWidget):
    def __init__(self, parent=None, flight_name="", flight_notes=""):
        ...
        right_lay.addWidget(QLabel("Write address or choose on map"))
        ...
        self.bridge.double_clicked.connect(self.on_double_click)
        self.bridge.marker_moved.connect(self.on_marker_moved)
        self.bridge.drop_received.connect(self.on_marker_dropped)
    ...
    def on_marker_dropped(self, kind, lat, lng):
        point = [lat, lng]
        if kind == "start":
            self.start_point = point
            self.start_input.setText(f"{lat:.6f}, {lng:.6f}")
        else:
            self.end_point = point
            self.end_input.setText(f"{lat:.6f}, {lng:.6f}")
    ...
    def on_double_click(self, lat, lng):
    ...
    def on_marker_moved(self, kind, lat, lng):
        point = [lat, lng]
        if kind == "start":
            self.start_point = point
            self.start_input.setText(f"{lat:.6f}, {lng:.6f}")
    ...
    def refresh_map(self):
        ...
        <script>
        var map = null, bridge = null, startMarker = null, endMarker = null;

        function initMap(){
            if (map) return;
            map = L.map('map', { doubleClickZoom: false })
                     .setView([38.290427, 21.794836], 15);
            ...
            map.on('dblclick', function(e){
                ...
                if (bridge) bridge.mapDoubleClicked(e.latlng.lat, e.latlng.lng);
            });
        }

        function addOrUpdateMarker(lat, lng, kind){
            const icon = L.icon({
                iconUrl: kind==='start'
            ...
            if (marker) {
                ...
                marker.on('dragend', function(e){
                    ...
                    if (bridge) bridge.markerMoved(kind, pos.lat, pos.lng);
                });
        ...
        window.addStart = function(lat,lng){ addOrUpdateMarker(lat,lng,'start'); };
        window.addEnd   = function(lat,lng){ addOrUpdateMarker(lat,lng,'end'); };
        ...
        if self.start_point:
            html = html.replace('</script>', f"addStart({self.start_point[0]}, {self.start_point[1]});</script>")
        if self.end_point:
            html = html.replace('</script>', f"addEnd({self.end_point[0]}, {self.end_point[1]});</script>")
...
# ----------------------------------------------------------------------
# Flight Page – RECORD .MP4 WITH mp4v (100% PLAYABLE)
# ----------------------------------------------------------------------
class FlightPage(QWidget):
    ...
    def start_auto_flight(self):
        ...
        QTimer.singleShot(4000, self.execute_flight)
    ...
    def execute_flight(self):
        ...
        if dist_m > 20:
            ...
            d_cm = int(dist_m * 100)
            steps = [
                (f"forward {d_cm}", 2000 + int(d_cm / 10)),
                ("left 180", 2500),
                (f"forward {d_cm}", 2000 + int(d_cm / 10)),
                ("land", 3000)
            ]
        self.run_steps(steps, 0)

    def run_steps(self, steps, idx):
        ...
        QTimer.singleShot(delay, lambda: self.run_steps(steps, idx + 1))
