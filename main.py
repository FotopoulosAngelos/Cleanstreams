...
class Homepage(QWidget):
    ...
    def update_home_status(self):
        ...
        if isinstance(battery, int):
            ...
            self.time_lbl.setText(f"{mins} min\nRemaining")
        QTimer.singleShot(5000, self.update_home_status)

    def open_flight_dialog(self):
        dialog = NewFlightDialog(self)
        if dialog.exec_():
            name, notes = dialog.get_data()
            if name:
                self.app.show_route_page(name, notes)
            else:
                QMessageBox.warning(self, "Error", "Flight name is required")
...
class NewFlightDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("New Flight")
        lay = QVBoxLayout(self)
        lay.setSpacing(15)

        self.name = QLineEdit()
        self.name.setPlaceholderText("Flight name")
        lay.addWidget(self.name)

        self.notes = QTextEdit()
        self.notes.setPlaceholderText("Flight notes")
        self.notes.setMaximumHeight(100)
        lay.addWidget(self.notes)

        save = RoundedButton("Save")
...
class FlightPage(QWidget):
    ...
    def execute_flight(self):
        """
        New flight logic:
        • distance ≤ 20 m  → original out-and-back round-trip
        • distance > 20 m  → custom “square-ish” route:
            takeoff → forward 20 m → left 90 → forward 4 m → left 90
            → forward 20 m → left 90 → forward 4 m → land
        """
        # ------------------------------------------------------------------
        # 1. Common start – takeoff + start recording
        # ------------------------------------------------------------------
        self.is_flying = True
        self.status_label.setText("Taking off…")
        self.start_recording()
        self.send_command("takeoff")
        time.sleep(4)                     # give the drone time to hover

        # ------------------------------------------------------------------
        # 2. Decide which path to fly
        # ------------------------------------------------------------------
        if self.distance <= 20.0:         # ---- ORIGINAL round-trip ----
            d_cm = int(self.distance * 100)
            self._fly_forward(d_cm)
            self.send_command("left 180")
            time.sleep(3)
            self._fly_forward(d_cm)

        else:                             # ---- YOUR custom route ----
            self._fly_forward(2000)      # 20 m = 2000 cm
            self._rotate_left(90)
            self._fly_forward(400)       # 4 m = 400 cm
            self._rotate_left(90)
            self._fly_forward(2000)      # 20 m again
            self._rotate_left(90)
            self._fly_forward(400)       # 4 m again
    ...
    # ----------------------------------------------------------------------
    # Helper methods – keep the sleeps you already used
    # ----------------------------------------------------------------------
    def _fly_forward(self, cm: int):
        """Send a forward command and wait the appropriate time."""
        self.send_command(f"forward {cm}")
        # 2 s base + 1 s per 100 cm (≈ 1 m/s)
        wait = 2 + cm / 100
        time.sleep(wait)

    def _rotate_left(self, degrees: int):
        """Rotate left and wait for the rotation to finish."""
        self.send_command(f"left {degrees}")
        # 90° takes ~2.5 s, 180° took 3 s in the original code
        time.sleep(3 if degrees >= 180 else 2.5)
